...
class Substitute(Scene):
    def construct(self):
        ...
        self.play(*[MoveToTarget(i) for i in formula[:4]],*[MoveToTarget(i) for i in formula[4:]])
        self.wait(2)
        ...
        ans_formula = TexMobject("\\emph{Thickness}",'\\emph{of}','\\emph{string}','=','{'+str(val_coil_length),'\\over',str(val_no_of_turns)+'}',f'{val_answer} mm')
        ...
        self.play(FadeOutAndShift(ans_formula[4:7],UP),FadeInFrom(ans_formula[7],RIGHT))
        self.wait()
        
        self.play(*[Uncreate(i) for i in [ans_formula[0],ans_formula[1],ans_formula[2],ans_formula[3],ans_formula[7]]])
        self.wait()
